...
const basicCalculator = (num1, num2, opp) => {
    if (opp === "add") { result= num1+num2;}
    else if (opp === "subtract"){ result= num1- num2;}
    else if(opp === "multiply") {result= num1* num2 ;}
    else if(opp === "divide") {result= num1 / num2 ;}
      return result; 
...
const createUsername = (first, last) => {
        ...
        for (i=1; i<=last.length; i++){
          ...
          return `${first.slice(0, 3)}${last.slice(0,3)}${count1+count2}`
...
const numArgs = ()=> {return arguments.length-1;}
